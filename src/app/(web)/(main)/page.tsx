...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const exists = filteredProductsByBrand.some(
  ...
  const deliveryCounts = useMemo(() => {
    return {
      ordered: shipmentContext.filter(
        (shipment) => shipment.status === "ordered",
      ).length,
      on_delivery: shipmentContext.filter(
        (shipment) => shipment.status === "on_delivery",
      ).length,
      arrived: shipmentContext.filter(
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 mt-1">
          Monitoring instan stok dan delivery berdasarkan brand/product
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white border border-gray-200 rounded-xl p-5">
          <h2 className="text-lg font-semibold text-gray-900 mb-2">
            Distribusi Total Stok per Status
          </h2>
          <p className="text-sm text-gray-500 mb-4">
            Tersedia, Ordered, On Delivery, dan Arrived
          </p>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5">
          ...
          <div className="space-y-3">
            ...
            <QtyRow
              ...
              label="Ordered"
              value={stockSummary.ordered}
              color="bg-amber-100 text-amber-700"
            />
            <QtyRow
              icon={<Truck className="w-4 h-4" />}
              label="On Delivery"
              value={stockSummary.onDelivery}
              color="bg-blue-100 text-blue-700"
            />
            <QtyRow
              icon={<Truck className="w-4 h-4" />}
              label="Arrived"
              value={stockSummary.arrived}
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-4">
          <div className="flex items-center justify-between gap-3">
            <h3 className="text-lg font-semibold text-gray-900">
              Detail Delivery - {toStatusLabel(selectedStatus)} (
          ...
          {detailShipments.length ? (
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      ETD {dayjs(shipment.etd).format("DD MMM YYYY HH:mm")} -
                      ETA {dayjs(shipment.eta).format("DD MMM YYYY HH:mm")}
                    </p>
                  </div>
            ...
            <p className="text-sm text-gray-500">
              Tidak ada delivery untuk filter saat ini.
            </p>
          )}
        </div>
      )}
...
function DeliveryCountCard({
  ...
  return (
    ...
    >
      ...
      <span
        className={`inline-flex mt-2 px-2.5 py-1 rounded-full text-sm font-semibold ${color}`}
      >
        {value}
      </span>
